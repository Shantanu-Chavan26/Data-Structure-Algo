...
void merge(int arr[], int s, int e) {
	...
	for(int i=0; i<lenLeft; i++) {
		left[i] = arr[k];//Left array madhe main array che values takat ahot
		k++;
	}
	...
	for(int i=0; i<lenRight; i++) {
		right[i] = arr[k]; //Right array madhe main array che values takat ahot
		k++;
	}
	...
	while(leftIndex < lenLeft &&  rightIndex < lenRight) {
		...
		if(left[leftIndex] < right[rightIndex] ) {
			arr[mainArrayIndex] =  left[leftIndex]; //Main Array madhe left array che values takat ahot.
			mainArrayIndex++;
			leftIndex++;
		}
		else {
			arr[mainArrayIndex] =  right[rightIndex]; //Main Array madhe Right array che values takat ahot. 
			mainArrayIndex++;
			rightIndex++;
		}
	}
	...
	//1 more step pending.
	//Deallocation of the dynamically added memory.
	delete[] left;
	delete[] right;
}
